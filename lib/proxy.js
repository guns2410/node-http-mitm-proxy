...
Proxy.prototype._onError = function(kind, ctx, err) {
  ...
  if (ctx) {
    ...
    if (ctx.proxyToClientResponse && !ctx.proxyToClientResponse.headersSent && !ctx.retriesAvailable) {
      ctx.proxyToClientResponse.writeHead(504, 'Proxy Error');
    }
    if (ctx.proxyToClientResponse && !ctx.proxyToClientResponse.finished && !ctx.retriesAvailable) {
...
Proxy.prototype._onHttpServerRequest = function(isSSL, clientToProxyRequest, proxyToClientResponse, retryCount = 0) {
  var self = this;
  var ctx = {
    isSSL: isSSL,
    ...
    retryRequest: function(retryCount) {
      const fn = self._onHttpServerRequest.bind(self, isSSL, clientToProxyRequest, proxyToClientResponse, retryCount);
      return fn()
    },
    use: function(mod) {
      if (mod.onError) {
